...
__global__ void binaryCrossEntropyCost(float* predictions, float* target, float* cost,
    ...
    if(row < pred_x_dim && col < pred_y_dim) {
        //printf("Row: %i, Col: %i , Pred: %f, Tar: %f \n", row, col, 0.0, 0.0);//predictions[row * pred_y_dim + col], target[row * pred_y_dim + col]);
        float partial_cost = target[row * pred_y_dim + col] * logf(predictions[row * pred_y_dim + col])
            + (1.0f - target[row * pred_y_dim + col]) * logf(1.0f - predictions[row * pred_y_dim + col]);
...
__global__ void dBinaryCrossEntropyCost(float* predictions, float* target, float* dY, 
    ...
    if (row < pred_x_dim && col < pred_y_dim) {
        dY[row * pred_y_dim + col] = -1.0 * ( target[row * pred_y_dim + col]/predictions[row * pred_y_dim + col] - (1-target[row * pred_y_dim + col])/(1-predictions[row * pred_y_dim + col]));
    }
}
